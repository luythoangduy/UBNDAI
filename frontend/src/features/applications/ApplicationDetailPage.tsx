...
function DocumentViewer({ detail }: { detail: CaseDetail }) {
  ...
  return <section className="am-evidence-panel" aria-label="Tài liệu gốc"><div className="am-panel-heading"><h2>Tài liệu gốc</h2><select aria-label="Chọn tài liệu" value={documentId} onChange={event => setDocumentId(event.target.value)}>{detail.documents.map(document => <option key={document.id} value={document.id}>{document.original_filename ?? document.document_type}</option>)}</select></div><div className="am-document-preview">{preview.isLoading ? 'Đang mở tài liệu…' : previewUrl ? <iframe title="Bản xem trước tài liệu" src={previewUrl} /> : <p>Không thể hiển thị tài liệu. Vẫn có thể đối chiếu dữ liệu OCR bên cạnh.</p>}</div><h3>Dữ liệu OCR</h3><dl className="am-field-list">{fields.data?.map(field => <div key={field.id}><dt>{fieldLabel(field.field_key)}</dt><dd>{field.normalized_value || field.raw_value}<small>Độ tin cậy {Math.round(field.confidence * 100)}%</small></dd></div>)}</dl></section>;
}
