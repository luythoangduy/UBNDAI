...
def append_worklog(repo_root: Path) -> None:
    """Append changed files to WORKLOG.md under today's date section."""
    worklog = repo_root / "WORKLOG.md"
    if not worklog.exists():
        return

    raw = git("git status --short")
    if not raw:
        return

    lines = [
        l for l in raw.splitlines()
        if l.strip() and "node_modules" not in l and ".ai-log" not in l
    ]
    if not lines:
        return
    ...
    user_name = git("git config user.name") or "unknown"
    user_email = git("git config user.email") or ""
    author = f"{user_name} <{user_email}>" if user_email else user_name
    files_md = "\n".join(f"  - `{l.strip()}`" for l in lines)
    entry = f"\n### [{time_str}] Files changed (auto) — {author}\n\n{files_md}\n"
    ...
    if date_header in content:
        ...
        if template_marker in content:
            idx = content.index(template_marker)
            content = content[:idx] + f"## {date_str}\n" + entry + "\n---\n\n" + content[idx:]
        else:
            content = content + f"\n---\n\n## {date_str}\n" + entry
...
def main():
    ...
    # On session end, write changed files to WORKLOG.md
    if entry.get("event") in ("Stop", "stop", "SessionEnd", "sessionEnd", "AfterModel"):
        try:
            repo_root = Path(git("git rev-parse --show-toplevel") or ".")
            append_worklog(repo_root)
        except Exception:
            pass
