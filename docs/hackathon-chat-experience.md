...
## Cửa sổ chat-first và draft workspace

Chat-first chạy tại `/chat` như một cửa sổ độc lập, không thay thế workspace biểu mẫu/OCR
tại `/citizen` hoặc cổng cán bộ tại `/officer`. Trong cửa sổ chat: mô tả nhu cầu → xem các
lựa chọn mẫu có provenance → chọn mẫu registry → bổ sung field → sinh bản nháp trong drawer
bên phải. Mẫu tìm được từ live source nhưng chưa có schema nội bộ chỉ cho phép mở nguồn,
không hiện nút sinh tự động.
