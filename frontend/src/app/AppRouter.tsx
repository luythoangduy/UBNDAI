...
export function OfficerLoginGate({ children }: { children: React.ReactNode }) {
  ...
  return <main className="management-login"><form onSubmit={login}><a className="brand-mark" href="/citizen" aria-label="Về trang chủ UBNDAI">AI</a><h1>Đăng nhập cổng cán bộ</h1><label>Tài khoản<input value={username} onChange={event => setUsername(event.target.value)} autoComplete="username" /></label><label>Mật khẩu<input type="password" value={password} onChange={event => setPassword(event.target.value)} autoComplete="current-password" />{password.length > 0 && password.length < 8 && <small className="management-login__hint">Mật khẩu cần tối thiểu 8 ký tự.</small>}</label>{error && <p className="am-error" role="alert">{error}</p>}<button className="am-button management-login__submit" disabled={busy || password.length < 8}>{busy ? 'Đang đăng nhập…' : 'Đăng nhập'}</button><small className="demo-hint">Tài khoản demo dùng mật khẩu <code>ChangeMe123!</code></small></form></main>;
}
