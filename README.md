...
Frontend công dân dùng chat-first workspace: mẫu được chọn ngay trong hội thoại, field còn
thiếu được đánh dấu trước khi sinh preview và draft drawer có thể đóng/mở lại. AI revision
được gửi tới `POST /api/v1/drafts/revise`; thay đổi luôn đi qua màn hình diff để người dùng
duyệt trước khi áp dụng. Endpoint này cần `LLM_API_KEY`/`ANTHROPIC_API_KEY` hợp lệ.
