...
import { api } from '../../api';
import { metricPoints, normalizeNamedMetrics, normalizeStatusMetrics } from './dashboardMetrics';
...
export default function DashboardPage() {
  ...
  return <><header className="officer-topbar"><div><h1>Tổng quan</h1><p>Thống kê tình hình xử lý hồ sơ</p></div><Link className="am-button" to="/officer/applications">Mở danh sách hồ sơ</Link></header><div className="officer-container"><section className="am-dashboard-filters" aria-label="Bộ lọc thời gian"><label>Từ ngày<input type="date" value={from} onChange={event => change('from', event.target.value)} /></label><label>Đến ngày<input type="date" value={to} onChange={event => change('to', event.target.value)} /></label><label>Khoảng thời gian<select value={granularity} onChange={event => change('granularity', event.target.value)}><option value="day">Theo ngày</option><option value="week">Theo tuần</option><option value="month">Theo tháng</option></select></label></section>{loading ? <div className="am-dashboard-skeleton" aria-live="polite" aria-label="Đang tổng hợp dữ liệu"><span className="sr-only">Đang tổng hợp dữ liệu…</span>{Array.from({ length: 6 }, (_, index) => <div className="am-skeleton" key={index}/>)}</div> : failed ? <div className="am-error" role="alert">Không thể tải đầy đủ dashboard. <button onClick={() => results.forEach(result => result.refetch())}>Thử lại</button></div> : <><section className="officer-kpi-grid am-six-kpis" aria-label="Chỉ số xử lý hồ sơ">{kpis.map(([label, value]) => <article className="officer-kpi" key={String(label)}><span>{label}</span><strong>{value ?? 0}</strong></article>)}</section><Suspense fallback={<div className="am-loading" aria-live="polite">Đang tải biểu đồ…</div>}><DashboardCharts timeseries={metricPoints(results[1].data).map(item => ({ date: item.date ?? item.period ?? '', total: item.total ?? item.value ?? item.count ?? 0, completed: item.completed ?? 0 }))} statuses={normalizeStatusMetrics(results[2].data)} types={normalizeNamedMetrics(results[3].data)} anomalies={normalizeNamedMetrics(results[4].data)} /></Suspense></>}</div></>;
}
